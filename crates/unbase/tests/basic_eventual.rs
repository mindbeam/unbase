...
async fn basic_eventual() {
    ...
    assert!(rec_a1.get_value("animal_sound").await.expect("retrieval").expect("has value") == "Moo", "New subject should be internally consistent");
...
//    debug!("Root Index = {:?}", context_b.get_subject_head_memo_ids(root_index_subject_id)  );
    ...
    assert_eq!(rec_a1.get_value("animal_sound").await.unwrap().unwrap(),   "Moo");
    assert_eq!(rec_a1.get_value("animal_type").await.unwrap(), None, "Should not yet have a value on Slab A for animal_type");
